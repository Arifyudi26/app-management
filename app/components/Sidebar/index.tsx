...
import logo from "@app/public/images/logo/logo.svg";
...
const Sidebar = ({ sidebarOpen, setSidebarOpen }: SidebarProps) => {
  ...
  function filterMenuGroups() {
    return menuGroups.map((group) => {
      const filteredMenuItems = group.menuItems.filter((item) => {
        if (!item.user_role) {
          return true;
        }
        if (
          Array.isArray(item.user_role) &&
          item.user_role.includes(user?.user?.role)
        ) {
          return true;
        }
        return false;
      ...
      >
        ...
        <div className="no-scrollbar flex flex-col overflow-y-auto duration-300 ease-linear">
          <nav className="mt-5 px-4 py-4 lg:mt-9 lg:px-6">
            {filterMenuGroups().map((group, groupIndex) => (
              <div key={groupIndex}>
